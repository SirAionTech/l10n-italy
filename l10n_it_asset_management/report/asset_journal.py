...
class Report(models.TransientModel):
    ...
    report_l10n_it_asset_ids = fields.One2many("report_asset_journal_asset", "report_id")
    ...
    def get_depreciations(self):
        ...
        if not self.show_dismissed_assets:
            domain += [
                ...
                ("l10n_it_asset_id.dismiss_date", ">=", self.date.replace(month=1, day=1)),
            ]
...
class ReportCategory(models.TransientModel):
    ...
    def generate_totals(self):
        for categ in self:
            ...
            report_deps = categ.report_l10n_it_asset_ids.mapped("report_depreciation_ids")
